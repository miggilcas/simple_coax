...
void PIDControllerZ::Load(sdf::ElementPtr _sdf, const std::string& prefix)
{
  gain_p = 15.0;
  gain_d = 50.0;
  gain_i = 0.0;
...
double PIDControllerZ::update(double new_input, double x, double dx, double dt, double Vmin)
{
  // limit command
  //if (limit > 0.0 && fabs(new_input) > limit) new_input = (new_input < 0 ? -1.0 : 1.0) * limit;
  ...
  // update control output
  output = gain_p * p + gain_d * d + gain_i * i + Vmin;
  return output;
}

void PIDControllerZ::reset()
{
  input = dinput = 0;
  p = d =i= output = 0;
}
